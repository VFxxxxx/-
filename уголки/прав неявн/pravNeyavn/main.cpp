...
double NeyavniyRight(double T, double c,double h, double tau, int N)
{
...
getNormL2("for t=0:",u,h,N,0, tau);
    ...
    while(t < T) {
        ...
        u_new[0] = v0(t-tau,c); //
        u_new[N - 1] = vL(t-tau,c,  h*k);

        for(int i = 1; i <= N - 1; i++)
            u_new[i] = (1. - 1./gamma)*u_new[i - 1] + u[i - 1] / gamma;
...
cout<<tau+h<<endl;
    ...
    h = L / (N - 1);
    if(tau >= h/c) { NeyavniyRight(T,c,h,tau,N);}
